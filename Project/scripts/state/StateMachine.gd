...
func _input(event: InputEvent) -> void: # For input that requires no checks for physics like whether actor is on ground
	current_state.Handle_input(event)
			
func _process(delta: float) -> void: # Can check for input on every actual frame output
	current_state.Update(delta)
		
func _physics_process(delta: float) -> void: # Can check input as well as physics at a constant rate
